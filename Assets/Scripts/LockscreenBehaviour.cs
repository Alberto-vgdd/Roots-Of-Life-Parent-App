...
public class LockscreenBehaviour : MonoBehaviour, IDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	// Factor used to calculate between screen dimensions and unity units
	private float f;
	...
	void Start () {
		...
		// Determine factor based on unity canvas size and actual screen size
		f = 1080.0f / Screen.width;
	...
	void OnValidate() {
		...
		f = 1080.0f / getMainGameViewSize ().x;
